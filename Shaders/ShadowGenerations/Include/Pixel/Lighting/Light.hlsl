...
float3 DiffuseBDRF(LightingParameters parameters, float3 light_direction, float3 light_color)
{
    float3 cos_light_normal = saturate(dot(light_direction, parameters.world_normal)) * parameters.shadow;

	if (parameters.shading_model.type == ShadingModelType_SSS)
	{
		SampleCDRF(parameters, light_direction, parameters.shadow, cos_light_normal);
	}
...
float3 SpecularBRDF(LightingParameters parameters, float3 light_direction, float3 light_color, bool enable_anisotropy)
{
    float3 halfway_direction = normalize(light_direction + parameters.view_direction);
    ...
    return light_color * cos_light_normal * saturate(fresnel * distribution * visibility) * parameters.shadow;
}
