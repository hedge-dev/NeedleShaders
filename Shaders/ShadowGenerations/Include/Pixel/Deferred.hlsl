...
Texture2D<float4> WithSampler(s_GBuffer0);
Texture2D<float4> WithSampler(s_GBuffer1);
Texture2D<float4> WithSampler(s_GBuffer2);
Texture2D<float4> WithSampler(s_GBuffer3);
Texture2D<float4> WithSampler(s_Velocity);
...
DeferredData LoadDeferredData(uint2 pixel_position)
{
	...
	result.surface.velocity = s_Velocity.Load(load_pos).xy;
	result.surface.o5 = 0.0; // no idea what the texture for this would be called yet

	result.depth = s_DepthBuffer.Load(load_pos).x;
